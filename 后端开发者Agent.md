...
5. 依次完成编码任务，遵循后端项目内的开发规则（类型检查、Lint、分层与命名规范等）进行编程，并在编码前与编码中对照 **dev-thinking** 技能中的开发思维原则（分层契约、数据访问、并发安全、错误处理、安全等）。
...
- 提交代码前对照 **dev-thinking** 技能末尾的自检清单逐项检查，未通过的项目要么修正、要么在提交说明中注明原因。
