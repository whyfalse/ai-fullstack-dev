...
> 说明：本仓库中的 `dev-manager-ai-space/`、`frontend-ai-space/`、`backend-ai-space/` 三个目录以及 `需求分析师Agent.md`、`架构师Agent.md`、`前端开发者Agent.md`、`后端开发者Agent.md`、`SKILL.md` 是**模板源文件**，仅用于初始化时读取，不需要复制到新项目。新项目只需复制 `appendix/` 目录，以及各 `*-ai-space/rules/` 和 `*-ai-space/skills/` 下的内容到对应位置。
...
- 创建项目管理目录下的 `setting.local.json` 文件，内容如下（将占位符替换为用户提供的实际路径，保留引号）：
    ```json
    {
        "frontend": {
...
- 在前端项目空间下创建 AI skills，按照 AI 工具的规则，复制 [skills 文件夹](./frontend-ai-space/skills/) 下的内容。
- 在前端项目根目录下创建 `setting.local.json`，记录全栈项目管理目录的根路径（即当前项目目录的绝对路径），内容如下：
    ```json
    {
        "devManagerPath": "<$devManagerPath>"
    }
    ```
...
- 在后端项目空间下创建 AI skills，按照 AI 工具的规则，复制 [skills 文件夹](./backend-ai-space/skills/) 下的内容。
- 在后端项目根目录下创建 `setting.local.json`，记录全栈项目管理目录的根路径（即当前项目目录的绝对路径），内容如下：
    ```json
    {
        "devManagerPath": "<$devManagerPath>"
    }
