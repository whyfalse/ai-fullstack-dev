...
- **Agent 不自动链式触发** —— 各阶段需用户手动调用，可审查可回退，但无法做到"一次输入、自动产出"。
- **关键步骤需人工介入** —— DDL 执行、数据库迁移落地、契约确认、路径配置等有副作用或不可逆的操作由开发者自行处理。所有 Agent 均不代替用户执行上述高风险操作。
- **契约前置依赖强** —— 前后端并行以契约稳定为前提；需求未稳即开发会因契约反复变更推高同步成本。
- **三空间松耦合** —— dev-manager 与前端、后端空间之间仅靠 `setting.local.json` 中的路径互相引用，配置错误或目录迁移会导致 Agent 找不到文件，且无自动校验。
- **任务粒度偏粗** —— 按"一次模块变更 = 一前端 + 一后端任务"拆分，细粒度拆解由开发者在 Agent 内部完成。
- **无自动化质量闭环** —— 产出测试用例文档，但不绑定 CI/CD、不自动运行测试、不自动校验契约一致性。
- **可追溯但不可自动回退** —— 变更日志完整，但回退需人工对照日志逐步执行。
