...
5. 分析`$req`，判断是否可以按模块拆分（例如：登录、token生成校验等功能可划分到鉴权模块）并判断模块的变更类型（新增/修改/删除），生成一条总览信息，进行人工确认，如果通过则进行下一步，未通过则重新分析。

6. 针对每个拆分出功能模块：
	- [ ] 生成一个模块名称，使用需求简介（中文，简练，避免重复，不加"模块"后缀），例如：`登陆鉴权`。
	...
	- [ ] 更新 `/requirements/module/catalog.md`：新增模块时追加一行，修改已有模块时更新对应行的简介/优先级/备注。
